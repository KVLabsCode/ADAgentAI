...
interface NetworkCredentialDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  networkName: NetworkName | null
  config: NetworkConfig | null
  onConnect: (networkName: NetworkName, credentials: Record<string, string>) => Promise<boolean>
  isConnecting: boolean
}

export function NetworkCredentialDialog({
  open,
  onOpenChange,
  networkName,
  config,
  onConnect,
  isConnecting,
}: NetworkCredentialDialogProps) {
  const [credentials, setCredentials] = React.useState<Record<string, string>>({})
  const [showPasswords, setShowPasswords] = React.useState<Record<string, boolean>>({})
  const [errors, setErrors] = React.useState<Record<string, string>>({})

  // Reset state when dialog opens with new network
  React.useEffect(() => {
    if (open && config) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!validate() || !networkName) return

    const success = await onConnect(networkName, credentials)
    if (success) {
      onOpenChange(false)
    }
  }

  if (!config || !networkName) return null
      ...
      <DialogContent className="sm:max-w-md sm:left-[calc(50%+var(--sidebar-width)/2)]">
        <DialogHeader>
          <div className="flex items-center gap-3">
            <NetworkLogo network={networkName} size="md" />
            <div>
