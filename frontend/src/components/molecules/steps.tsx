...
export const StepsTrigger = ({
  ...
  className,
  leftIcon,
  swapIconOnHover: _swapIconOnHover = true,
  ...props
}: StepsTriggerProps) => (
  <CollapsibleTrigger
    className={cn(
      "group/steps text-muted-foreground hover:text-foreground flex cursor-pointer items-center gap-2 text-sm transition-colors",
      className
    )}
    {...props}
  >
    ...
    <span>{children}</span>
    <ChevronDown className="size-4 text-muted-foreground/70 transition-transform group-data-[state=open]/steps:rotate-180" />
  </CollapsibleTrigger>
)
